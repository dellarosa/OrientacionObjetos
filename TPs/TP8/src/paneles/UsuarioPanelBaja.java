...
public class UsuarioPanelBaja extends JPanel{
	...
	{			
		try
		{
			...
			panelResto.add(panelGestor.crearPanelOpcion("Ingrese UserName de usuario a eliminar", tfIngreso));
            ...
            public void actionPerformed(ActionEvent evt) {
            	try
            	{
	            	if(tfIngreso.getText().equals(""))
	            	{	
	            		JOptionPane.showMessageDialog(frame, "DATOS VACIOS");
	            	}else
	            	{
	            		Usuario usuarioBaja=usuarioBo.buscarUsuarioPorApodo(tfIngreso.getText());
	            		if(usuarioBaja==null)
						{	
	            		...
	            		int response=JOptionPane.showConfirmDialog(frame, "ELIMINAR USUARIO "+tfIngreso.getText()+"?", "Confirm",JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE);
