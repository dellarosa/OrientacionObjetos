...
public class AceitePanelModificacion extends JPanel{
	...
	private JTextField tfTipoAceite;
	private JTextField tfCantLitros;
	...
	public AceitePanelModificacion(final Handler handler)
	{
		...
		JPanel panelTitulo = panelGestor.crearPanelBorderConTitulo(new BorderLayout(),null,Color.black,new Dimension(400,50),"MODIFICACION DE ACEITE",JLabel.CENTER,new Font(Font.SERIF,Font.BOLD,15),Color.white);
		...
		panelCentro.add(BorderLayout.SOUTH,panelGestor.crearPanelOpcion("Ingrese id Autoparte del Aceite a modificar", tfIngreso));
	...
	private Aceite buscarAceite() {
		...
		{	
			...
			JOptionPane.showMessageDialog(handler.getFrame(), "Error al buscar Aceite ID autoparte", "Error", JOptionPane.ERROR_MESSAGE);
	...
	private JPanel crearFormularioModificacion() {
		...
		tfTipoAceite=panelGestor.crearTextField("",20,Definiciones.line_blackline,new Font(Font.SERIF,-1,12),Color.white,JTextField.LEFT_ALIGNMENT);
		tfCantLitros=panelGestor.crearTextField("",20,Definiciones.line_blackline,new Font(Font.SERIF,-1,12),Color.white,JTextField.LEFT_ALIGNMENT);
		...
		JLabel labelTipoAceite=panelGestor.crearLabel("Tipo Aceite("+aceite.getTipoAceite()+")",new Font(Font.SERIF,Font.BOLD,14),JLabel.RIGHT_ALIGNMENT,null,Definiciones.line_blackline);
		JLabel labelCantLitros=panelGestor.crearLabel("Cant. Litros("+aceite.getCantidadlitros()+")",new Font(Font.SERIF,Font.BOLD,14),JLabel.RIGHT_ALIGNMENT,null,Definiciones.line_blackline);
		
		JTextField[] textFields=new JTextField[]{tfMarca,tfModelo,tfCosto,tfCantDisp,tfTipoAceite,tfCantLitros};
		JLabel[] labels=new JLabel[]{labelMarca,labelModelo,labelCosto,labelCantDisp,labelTipoAceite,labelCantLitros};
	...
	private void modificarAutoparte() {
		...
		this.setLayout(new BorderLayout());					
		JPanel panelTitulo = panelGestor.crearPanelBorderConTitulo(new BorderLayout(),null,Color.black,new Dimension(600,50),"MODIFICACION DE ACEITE",JLabel.CENTER,new Font(Font.SERIF,Font.BOLD,15),Color.white);		
        ...
        public void actionPerformed(ActionEvent evt) {
        	try
    		{
    			...
    			if(handler.updateAceite(aceite))
    			{	
    				if(handler.updateAutoparte(autoparte))
    				{
    					JOptionPane.showMessageDialog(handler.getFrame(), "ACEITE MODIFICADO CORRECTAMENTE");
    				}else
    				{
    			...
    			{
    				JOptionPane.showMessageDialog(handler.getFrame(), "FALLO MODIFICACION ACEITE NO ENCONTRADO EN DB");
    			}				
    			handler.backToPrincipal();
    		}catch (MiException e1) {
    			JOptionPane.showMessageDialog(handler.getFrame(), "Error al modificar aceite", "Error", JOptionPane.ERROR_MESSAGE);
    			handler.backToPrincipal();
    		}
    	}});
	...
	private void guardarModificaciones() {
		...
		aceite.setTipoAceite(!tfTipoAceite.getText().equals("")?tfTipoAceite.getText():aceite.getTipoAceite());
		aceite.setCantidadlitros(!tfCantLitros.getText().equals("")?Integer.valueOf(tfCantLitros.getText()):aceite.getCantidadlitros());
	}
	private void llenarAutoparte(){
		try
		{
			autoparte=handler.buscarAutopartePorId(aceite.getId());
		} catch (MiException e1) {
